...
function InterfaceRow({ iface }: { iface: OSPFInterface }) {
  ...
  return (
    <div className="flex flex-col gap-0.5 bg-secondary/40 rounded-md px-2.5 py-2 border border-border/30">
      ...
      {iface.drAddress && (
        <div className="text-[10px] text-muted-foreground/70 font-mono">
          DR: {iface.drAddress}{iface.bdrAddress ? ` / BDR: ${iface.bdrAddress}` : ""}
        </div>
      )}
    </div>
  )
            ...
            <div className="flex flex-col gap-0">
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(data as OSPFRouter).lsaTypes.map((type) => (
                      <span key={type} className="text-[10px] bg-secondary px-2 py-0.5 rounded-sm text-secondary-foreground">
                        {type}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-col gap-1">
                    {(data as OSPFRouter).stubNetworks.map((n) => (
                      <span key={n} className="font-mono text-xs text-secondary-foreground bg-secondary/50 px-2 py-1 rounded-sm">
                        {n}
                      </span>
                    ))}
                  </div>
              ...
              {(data as OSPFRouter).learnedRoutes?.length > 0 && (
                <div className="mt-3">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {(data as OSPFRouter).learnedRoutes.map((r, i) => (
                      <LearnedRouteRow key={i} route={r} />
                    ))}
                  </div>
                </div>
              )}
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-col gap-1">
                    {(data as OSPFNetwork).attachedRouters.map((r) => (
                      <span key={r} className="font-mono text-xs text-secondary-foreground bg-secondary/50 px-2 py-1 rounded-sm">
                        {r}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {[
                    ...
                    >
                      ...
                      <span className="font-mono text-sm font-bold" style={{ color: isAsymmetric ? "#f87171" : color }}>
                        {cost}
                      </span>
                    </div>
                  ))}
